...
왠만하면 `val` 위주로 적용해서 안전성을 높이는 코딩을 지향해야하고, 꼭 가변적으로 할당되어야만 하는 변수에는 `var`를 사용하도록 합시다.
...
그러나 `null`을 허용하는 변수는 기본형으로 변환되는 참조형일지라도 자기만의 값을 힙에 할당하고 그것을 가리키는 주소를 `스택공간`에 할당합니다.
